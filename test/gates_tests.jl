...
@testset "Gates tests: elementary universal gates in gates.jl" begin
    ...
    U3_negpi = QCO.get_unitary("U3_2", 2, angle=[-π/2, 0, 0])
    U3_pi    = QCO.get_unitary("U3_2", 2, angle=[ π/2,  0, 0])
    @test isapprox(QCO.CRYGate(π), CNOT12 * U3_negpi * CNOT12 * U3_pi, atol=tol_0)

    # Identity tests
    Id     = QCO.IGate(2)
    CRXRev = QCO.get_unitary("CRX_2_1", 2, angle=π)
    @test isapprox(Id, CRXRev^4, atol=tol_0)
    CRYRev = QCO.get_unitary("CRY_2_1", 2, angle=π)
    @test isapprox(Id, CRYRev^4, atol=tol_0)
    CRZRev = QCO.get_unitary("CRZ_2_1", 2, angle=π)
